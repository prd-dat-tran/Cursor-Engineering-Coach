  ...
  private deriveContextAntiPatterns(
    ...
    if (activeNoContext.length > 0) {
      ...
      patterns.push({
        ...
        suggestion: 'Add an AGENTS.md or .cursor/rules/<topic>.mdc file to your most active workspaces. This is the single highest-leverage action for better Cursor outputs.',
    ...
    for (const [harness, score] of Object.entries(contextProvision)) {
      if (score.score < 20 && score.totalRequests >= 50) {
        ...
        patterns.push({
          ...
          suggestion: `In ${harness}, reference files with @file/@folder, add a .cursor/rules/*.mdc or AGENTS.md, and create skills for repeatable workflows to provide richer context.`,
          examples: [],
          details: provDetails,
          weeklyHist: { labels: [], counts: [] },
        });
